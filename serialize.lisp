  ...
  (cond ((null symbol) 
	...
	(t
	 (let* ((symbol-name (sb-ext:string-to-octets (symbol-name symbol)))
		...
		(vec (kyoto-persistence:kcmalloc sz))
		(position 0))
	   (setf (mem-aref vec :unsigned-char position) +symbol+)
	   (dotimes (i (length encoded-length))
	     (setf (mem-aref vec :unsigned-char (incf position)) (aref encoded-length i)))
	   (setf (mem-aref vec :unsigned-char (incf position)) +string+)
	   (dotimes (i (length symbol-encoded-length))
	     (setf (mem-aref vec :unsigned-char (incf position))
